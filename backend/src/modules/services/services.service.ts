...
export class ServicesService {
  ...
  async compareServicesByInstance(sourceAppInstanceId: string, targetAppInstanceId: string): Promise<any> {
    this.logger.debug(`Comparing services between app instances: ${sourceAppInstanceId} -> ${targetAppInstanceId}`);

    // Fetch services from both app instances
    const [sourceServices, targetServices] = await Promise.all([
      this.getServicesByAppInstance(sourceAppInstanceId),
      this.getServicesByAppInstance(targetAppInstanceId),
    ]);
