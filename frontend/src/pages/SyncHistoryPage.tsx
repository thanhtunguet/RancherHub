...
import { Card, Table, Tag, Typography, Select, Space, Input, Tooltip, Modal } from 'antd';
import { HistoryIcon, ArrowRightIcon, ClockIcon, AlertTriangleIcon } from 'lucide-react';
...
export function SyncHistoryPage() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<SyncHistory | null>(null);

  const { data: syncHistory, isLoading } = useQuery({
    queryKey: ['detailed-sync-history'],
    queryFn: () => api.get<SyncHistory[]>('/api/services/sync/history/detailed').then(res => res.data),
  ...
  const filteredHistory = syncHistory?.filter(record => {
    const matchesSearch = !searchTerm || 
      ...
      record.sourceEnvironmentName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      record.targetEnvironmentName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const columns = [
    {
      ...
      render: (name: string, record: SyncHistory) => (
        <div>
          ...
          <div className="text-xs text-gray-500">{record.workloadType || 'N/A'}</div>
        </div>
      ),
    },
    {
      ...
      render: (record: SyncHistory) => (
        <div className="flex items-center gap-2">
          <div className="text-center">
            <div className="text-sm font-medium">{record.sourceEnvironmentName || 'Unknown'}</div>
            <div className="text-xs text-gray-500">{record.sourceCluster || 'N/A'}/{record.sourceNamespace || 'N/A'}</div>
          </div>
          <ArrowRightIcon size={16} className="text-gray-400" />
          <div className="text-center">
            <div className="text-sm font-medium">{record.targetEnvironmentName || 'Unknown'}</div>
            <div className="text-xs text-gray-500">{record.targetCluster || 'N/A'}/{record.targetNamespace || 'N/A'}</div>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (record: SyncHistory) => (
        <Space>
          <a onClick={() => setSelectedRecord(record)} className="text-blue-500 hover:text-blue-700">
            View Details
          </a>
          {record.error && (
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <Title level={2} className="m-0">Sync History</Title>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded mt-1">
                <div>Environment: {selectedRecord.sourceEnvironmentName || 'Unknown'}</div>
            ...
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded mt-1">
                <div>Environment: {selectedRecord.targetEnvironmentName || 'Unknown'}</div>
            ...
            <div>
              ...
              <div className="bg-gray-50 p-3 rounded mt-1">
                {selectedRecord.previousImageTag && (
                  <div>Previous: <code>{selectedRecord.previousImageTag}</code></div>
                )}
                <div>New: <code>{selectedRecord.newImageTag}</code></div>
              </div>
            </div>
            ...
            {selectedRecord.configChanges && (
              <div>
                ...
                <div className="bg-gray-50 p-3 rounded mt-1">
                  <pre className="text-xs">{JSON.stringify(selectedRecord.configChanges, null, 2)}</pre>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div>{dayjs(selectedRecord.timestamp).format('YYYY-MM-DD HH:mm:ss')}</div>
            </div>
          </div>
        )}
      </Modal>
