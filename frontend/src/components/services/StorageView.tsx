...
import { CloudServerOutlined, ReloadOutlined, DatabaseOutlined } from '@ant-design/icons';
...
export const StorageView: React.FC<StorageViewProps> = ({ style }) => {
  ...
  const [selectedAppInstance, setSelectedAppInstance] = useState<string | null>(null);
  ...
  const fetchAppInstanceTree = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load app instances');
    } finally {
  ...
  const fetchServicesWithSizes = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/services/with-image-sizes/${selectedAppInstance}`);
      if (!response.ok) {
        throw new Error('Failed to fetch services with image sizes');
      }
  ...
  const getSelectedAppInstanceInfo = () => {
    for (const env of appInstanceTree) {
      const instance = env.appInstances.find(ai => ai.id === selectedAppInstance);
      if (instance) {
  ...
  const totalSize = services.reduce((sum, service) => sum + (service.imageSize || 0), 0);
      ...
      render: (_, record) => (
        <div style={{ textAlign: 'center' }}>
          <Progress
            type="circle"
            size={50}
            percent={record.replicas > 0 ? Math.round((record.availableReplicas / record.replicas) * 100) : 0}
            format={() => `${record.availableReplicas}/${record.replicas}`}
            strokeColor={record.availableReplicas === record.replicas ? '#52c41a' : '#faad14'}
          />
        </div>
      ),
    },
        ...
        <Card size="small">
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: '300px' }}>
              <Text strong style={{ marginRight: '8px' }}>App Instance:</Text>
              <Select
          ...
          <>
            {loading ? (
              <div style={{ textAlign: 'center', padding: '40px' }}>
                <Spin size="large" />
                <p style={{ marginTop: '10px' }}>Loading services with image sizes...</p>
              </div>
            ) : error ? (
              <Alert
                message="Error"
                ...
                showIcon
                action={
                  <Button onClick={fetchServicesWithSizes} type="primary" size="small">
                    Retry
                  </Button>
                }
              />
