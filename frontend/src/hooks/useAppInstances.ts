...
export const useCreateAppInstance = () => {
  ...
  return useMutation({
    ...
    onSuccess: (newAppInstance) => {
      ...
      message.success(
        `App instance "${newAppInstance.name}" created successfully`
      );
    },
    onError: (error: any) => {
      message.error(
        error.response?.data?.message || "Failed to create app instance"
      );
    },
  });
};
...
export const useUpdateAppInstance = () => {
  ...
  return useMutation({
    ...
    onSuccess: (updatedAppInstance) => {
      queryClient.invalidateQueries({ queryKey: ["app-instances"] });
      message.success(
        `App instance "${updatedAppInstance.name}" updated successfully`
      );
    },
    onError: (error: any) => {
      message.error(
        error.response?.data?.message || "Failed to update app instance"
      );
    },
  });
};
...
export const useDeleteAppInstance = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      message.error(
        error.response?.data?.message || "Failed to delete app instance"
      );
    },
  });
};
