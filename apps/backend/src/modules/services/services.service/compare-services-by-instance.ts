...
export async function compareServicesByInstance(service: ServicesService, sourceAppInstanceId: string, targetAppInstanceId: string): Promise<any> {
  service.logger.debug(`Comparing services between app instances: ${sourceAppInstanceId} -> ${targetAppInstanceId}`);
